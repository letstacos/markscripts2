...
// Audio assets
const sHooya = new Audio('/hooya.wav'); // play on hit
sHooya.volume = 0.9;
const sDing = new Audio('/dingaling.wav'); // play on kill
sDing.volume = 0.9;
const sBell = new Audio('/Church Bell - Sound Effect 4.mp3'); // play on life lost
sBell.volume = 0.9;

// Background music playlist (plays in order at fixed 25% volume)
const _bgTracks = [
  '/Casin 4.mp3',
  '/Messages from the Stars 4.mp3',
  '/【HQ】 Lady (Hear me Tonight) - Modjo.mp3'
];
let _bgIndex = 0;
let _bgAudio = null;
function _playNextBg(){
  try {
    if(_bgAudio){
      _bgAudio.removeEventListener('ended', _playNextBg);
      try { _bgAudio.pause(); } catch(e){}
    }
    const path = _bgTracks[_bgIndex % _bgTracks.length];
    _bgAudio = new Audio(path);
    _bgAudio.volume = 0.25;
    _bgAudio.loop = false;
    // in case the file errors, skip to next
    _bgAudio.addEventListener('error', ()=>{
      _bgIndex = (_bgIndex + 1) % _bgTracks.length;
      setTimeout(_playNextBg, 120);
    });
    _bgAudio.addEventListener('ended', ()=>{
      _bgIndex = (_bgIndex + 1) % _bgTracks.length;
      // slight delay before starting next to avoid abrupt transitions
      setTimeout(_playNextBg, 120);
    });
    // attempt to play; if blocked by autoplay, we'll resume on first user interaction
    const p = _bgAudio.play();
    if(p && p.catch){
      p.catch(()=>{ /* autoplay blocked; will resume on interaction */ });
    }
  } catch(e){
...
_playNextBg();
// If autoplay is blocked, resume playback on first user interaction (click/touch/keydown)
function _resumeBgOnInteraction(){
  try {
...
['pointerdown','touchstart','keydown','click'].forEach(evt=>{
  window.addEventListener(evt, function _onceResume(){
    _resumeBgOnInteraction();
    // also ensure we remove this handler after first use
    window.removeEventListener(evt, _onceResume);
  }, {passive:true});
});
// shop UI elements
...
// horizontal rotation speed for the sky dome (radians/sec)
let skyRotateSpeed = 0.06;
// Global references for runtime map switching
let sky, allskyTex, groundTex, floorVis, floorHit, smallVis;
let originalSkyPath = '/allsky-free-its-allsky-but-free-a-high-quality-skybox-pack-v0-9blv8ups6sy21.webp';
let originalGroundPath = '/wall-texture-with-yellow-sand-bricks-photo.jpg';
let usingMap2 = false;
...
function applySavedState(){
  ...
  if(s.hotbar && Array.isArray(s.hotbar)){
    for(let i=0;i<s.hotbar.length;i++){
      const id = s.hotbar[i];
      const slotIndex = i+1;
      // Never restore or populate slot 2 from saved state – keep it reserved/empty.
      if(slotIndex === 2){
      ...
      const slot = document.querySelector(`.hotbar-slot[data-slot="${slotIndex}"]`);
      if(slot){
        if(id){
...
function registerKill(){
  // In practice mode kills should not be recorded.
  ...
  // play kill sound
  try { sDing.currentTime = 0; sDing.play().catch(()=>{}); } catch(e){}
 ...
 // practice mode flag: when true, start a persistent normal round (no waves) until player restarts from pause
...
function init(){
  ...
  // store global sky texture + mesh so we can swap later
  allskyTex = texLoader.load(originalSkyPath);
  ...
  // store global ground texture and material for swapping
  groundTex = texLoader2.load(originalGroundPath);
  ...
  floorHit = new THREE.Mesh(floorGeo, new THREE.MeshBasicMaterial({visible:false}));
  ...
  smallVis = new THREE.Mesh(SMALL_GEO, floorMat.clone());
  ...
  for(const e of enemies){
    ...
    if(distXZ <= reach){
      ...
      if(angle < Math.PI/3){
        ...
        // play hit sound
        try { sHooya.currentTime = 0; sHooya.play().catch(()=>{}); } catch(e){}
      }
    }
  }
}
...
function animate(){
  ...
  // slowly rotate the sky dome for a subtle animated backdrop (works for both map textures)
  if(typeof dt !== 'undefined' && sky && sky.rotation){
    sky.rotation.y += dt * skyRotateSpeed;
  }
...
function handleWaveCompletion(){
  // If practicing, do not trigger upgrade/cannon flows — keep one long classic wave.
  if(practiceMode){
    // reset per-wave counters and keep gameplay running as a continuous normal wave
    killsThisWave = 0;
    killsSinceWaveStart = 0;
    // ensure no cannon activity during practice
    currentWaveType = 'normal';
    cannonActive = false;
    for(const cb of cannonballs){ if(cb) scene.remove(cb); }
    cannonballs = [];
    // do not show upgrade UI or modify lives/active upgrades
    suppressKills = false;
...
function spawnNextCannonball(count = 1){
  // If practicing, do not spawn cannonballs — practice is a continuous normal wave.
  if(practiceMode) return;
...
function playerDie(reason = 'fell'){
  ...
  // play life-lost bell sound
  try { sBell.currentTime = 0; sBell.play().catch(()=>{}); } catch(e){}
...
window.addEventListener('keydown', (ev)=>{
  ...
  if(now - _lastEscAt <= _ESC_WINDOW_MS){
    ...
    if(paused){
      // show overlay as pause indicator, hide respawn button while paused, conditionally show restart round option
      overlayText.textContent = 'Paused';
      if(respawnBtn) respawnBtn.style.display = 'none';
      // hide Restart Round during practice mode
      if(restartBtn) restartBtn.style.display = (practiceMode ? 'none' : '');
      overlay.style.display = 'flex';
    } else {
      // unpause: hide overlay and restore respawn button visibility
      overlay.style.display = 'none';
      if(respawnBtn) respawnBtn.style.display = '';
      // always hide Restart Round while playing
  ...
  function playerGunShoot(){
    ...
    function applyGunImpulse(targetMesh, power){
      ...
      // play hit sound for gun impulse
      try { sHooya.currentTime = 0; sHooya.play().catch(()=>{}); } catch(e){}
    }
 ...
 // G key: toggle pause (single-press) — replaces previous gun-key behavior
 window.addEventListener('keydown', (ev)=>{
   if(ev.code !== 'KeyG') return;
   // toggle paused state and show/hide overlay similar to Escape double-press unpause flow
   paused = !paused;
   const restartBtn = document.getElementById('restartRoundBtn');
   if(paused){
     overlayText.textContent = 'Paused';
     if(respawnBtn) respawnBtn.style.display = 'none';
     // hide Restart Round while practicing
     if(restartBtn) restartBtn.style.display = (practiceMode ? 'none' : '');
     overlay.style.display = 'flex';
   } else {
     overlay.style.display = 'none';
 ...
 function playerThrowGrenade(){
  ...
  // Handler for "Clear" buttons next to each upgrade: reset related pending/permanent upgrade state and cooldown reduction.
  (function(){
    document.addEventListener('click', (ev)=>{
      const btn = ev.target && ev.target.closest && ev.target.closest('.clearUpgradeBtn');
      if(!btn) return;
      const upKey = btn.dataset.upgrade;
      try {
        // Clear matching pending/permanent flags or counters
        if(upKey === 'jumpBoost'){
          permanentUpgrades.jumpBoost = false;
          permanentUpgrades.jumpCount = 0;
          pendingUpgrades.jumpBoost = false;
        } else if(upKey === 'slowMo'){
          pendingUpgrades.slowMo = false;
        } else if(upKey === 'autoFace'){
          pendingUpgrades.autoFace = false;
        }

        // Also reset cooldown-related permanent purchases and reduction for safety
        permanentUpgrades.cooldownCount = 0;
        cooldownReduction = 0;
        ...
        // Visual feedback
        btn.style.outline = '2px solid rgba(255,80,80,0.35)';
        setTimeout(()=>{ btn.style.outline = ''; }, 420);

        const upLives = document.getElementById('upgradeLives');
        if(upLives) upLives.textContent = String(totalKills || 0);

        // brief confirmation
        try{ alert('Cleared upgrades and reset cooldown reductions.'); }catch(e){}
      } catch(e){
        console.warn('clearUpgradeBtn handler error', e);
      }
    });
  })();
...
respawnBtn.addEventListener('pointerdown', (e)=>{
 ...
 // Practice PVP button handler on the overlay: start a persistent normal round (no waves) until you restart from pause
 const practiceBtn = document.getElementById('practiceBtn');
 if(practiceBtn){
   practiceBtn.addEventListener('pointerdown', (e)=>{
     e.preventDefault();
     // enable practice mode
     practiceMode = true;
     // while practicing, remove the Restart Round control from the pause overlay
     try { const restartBtnLocal = document.getElementById('restartRoundBtn'); if(restartBtnLocal) restartBtnLocal.style.display = 'none'; } catch(e){}
     paused = false;
     isGameOver = false;
     overlay.style.display = 'none';
     // ensure player is centered and invulnerable briefly
     camera.position.set(0,18,0);
     camera.rotation.set(0,0,0,'YXZ');
     invulnerableUntil = performance.now() + 1200;
     // set round state: always normal, no cannon waves and no wave completion flow
     currentWaveType = 'normal';
     cannonActive = false;
     awaitingUpgrade = false;
     suppressKills = false;
     // clear any enemies/cannonballs and spawn an initial persistent batch
     for(const cb of cannonballs){ if(cb) scene.remove(cb); }
     cannonballs = [];
     for(const e of enemies){
       if(e.mesh){ scene.remove(e.mesh); e.mesh._despawned = true; }
     ...
     enemies = [];
     // spawn a larger continuous group to practice against
     respawnEnemies(6);
   });
 }
...
if(restartRoundBtn){
 ...
 function restartRoundFromPause(){
   // exiting any practice session if active
   practiceMode = false;
   // restore the Restart Round control in the pause overlay when leaving practice
   try { const restartBtnLocal = document.getElementById('restartRoundBtn'); if(restartBtnLocal) restartBtnLocal.style.display = ''; } catch(e){}
   // unpause the game and hide overlay
   paused = false;
   overlay.style.display = 'none';
   // respawn normal starter enemies
   respawnEnemies(2);
...
function openShop(){
  ...
  // visually compact the hotbar while the shop is open
  try {
    const hb = document.getElementById('hotbar');
    if(hb) hb.classList.add('shop-open');
  } catch(e){}
  shopSelectedItem = null;
  if(shopSelectedLabel) shopSelectedLabel.textContent = 'None';
...
function closeShop(){
  ...
  // restore hotbar size/opacity after closing shop
  try {
    const hb = document.getElementById('hotbar');
    if(hb) hb.classList.remove('shop-open');
  } catch(e){}
  shopSelectedItem = null;
  if(shopSelectedLabel) shopSelectedLabel.textContent = 'None';
}
...
// Clear All Upgrades button in the shop: remove pending, active, and permanent upgrades and restore shop buttons
const clearAllBtn = document.getElementById('clearAllUpgrades');
if(clearAllBtn){
  clearAllBtn.addEventListener('pointerdown', (e)=>{
    e.preventDefault();
    // Confirm the destructive action
    const ok = confirm('Clear ALL upgrades and reset purchased upgrade state? This cannot be undone.');
    if(!ok) return;

    // Clear pending and active upgrades
    pendingUpgrades = { slowMo:false, jumpBoost:false, autoFace:false };
    activeUpgrades = { slowMo:false, jumpBoost:false, autoFace:false };

    // Reset permanent upgrades counters and related reductions
    permanentUpgrades = {
      jumpBoost: false,
      speedCount: 0,
      jumpCount: 0,
    ...
    // Clear purchasedItems map and re-enable all shop item buttons
    purchasedItems = {};
    document.querySelectorAll('.shop-item[data-item]').forEach(btn=>{
      try {
        btn.disabled = false;
        btn.style.opacity = '';
        // remove 'Owned' suffix if present
        if(btn.title) btn.title = btn.title.replace(' • Owned','');
      } catch(e){}
    });

    // Reset upgrade buttons visuals and allow repurchase where applicable
    document.querySelectorAll('.upgradeBtn').forEach(b=>{
      b.disabled = false;
      b.style.opacity = '';
    });
    ...
    // Update UI: refresh upgrade lives counter, shop currency and inventory UI
    try { 
      const upLives = document.getElementById('upgradeLives');
      if(upLives) upLives.textContent = String(totalKills || 0);
      updateShopKillsDisplay();
    ...
    try { alert('All upgrades cleared and shop items re-enabled.'); } catch(ex){}
  });
}
...
if(shopScreen){
  shopScreen.addEventListener('click', (ev)=>{
    ...
    if(btn && btn.dataset && btn.dataset.item){
      ...
      if(cost > 0){
        if(totalKills >= cost){
          totalKills -= cost;
          // purchased items go to inventory; do not auto-select/equip the bought item
          shopSelectedItem = null;
          if(shopSelectedLabel) shopSelectedLabel.textContent = 'None';
          ...
          if(itemId === 'speed_upgrade' || itemId === 'jump_upgrade' || itemId === 'cooldown_upgrade'){
            ...
            // determine item category so we can allow repeated buys for potions/handhelds
            const itemType = (itemStats[itemId] && itemStats[itemId].type) ? itemStats[itemId].type : '';

            // add to inventory (except extra_slot handled separately)
            if(itemId !== 'extra_slot'){
              inventory.push(itemId);
            }

            // Extra slot purchase (one-time)
            if(itemId === 'extra_slot'){
              addExtraHotbarSlot();
              ...
              btn.style.opacity = '0.55';
              btn.title = (btn.title ? btn.title + ' • Owned' : 'Owned');
            } else if(itemType === 'potion' || itemType === 'handheld'){
              // Allow unlimited purchases: do NOT mark as permanently owned or disable button.
              // Give a brief purchase feedback but leave the button active for repeat buys.
              btn.style.outline = '2px solid rgba(40,200,120,0.45)';
              btn.style.opacity = '0.78';
              setTimeout(()=>{ btn.style.outline = ''; btn.style.opacity = ''; }, 420);
            } else {
              // For other non-consumable items, mark as purchased and disable to prevent re-buy.
  ...
  if(redeemBtn && codeInput){
    redeemBtn.addEventListener('click', (e)=>{
      ...
      if(code === 'ineedkills'){
        totalKills = (totalKills || 0) + 5236381;
...
function renderInventoryList(){
  ...
  inventory.forEach((id, idx) => {
    ...
    placeBtn.addEventListener('click', ()=> {
      ...
      const s = prompt('Place into slot number (1-6). Note: slot 2 and slot 3 are reserved (slot 2 always reserved; slot 3 reserved for grenades). Choose another slot to swap.');
      const slotNum = Number(s);
      if(!slotNum || isNaN(slotNum) || slotNum < 1 || slotNum > 6){ alert('Invalid slot'); return; }
      // disallow placing into reserved slot 2
      if(slotNum === 2){ alert('Slot 2 is reserved and cannot hold items.'); return; }
 ...
 // Map switch UI wiring (pause overlay)
 const map2Btn = document.getElementById('map2Btn');
 const map1Btn = document.getElementById('map1Btn');
 function _swapSkyTexture(path){
   try {
     const loader = new THREE.TextureLoader();
     const t = loader.load(path, (tex)=>{
       tex.encoding = THREE.sRGBEncoding;
       tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
       tex.repeat.set(1,1);
       // update scene background and sky mesh material
   ...
   } catch(e){ console.warn('swapSky failed', e); }
 }
 function _swapGroundTexture(path){
   try {
     const loader = new THREE.TextureLoader();
     const t = loader.load(path, (tex)=>{
       tex.encoding = THREE.sRGBEncoding;
       tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
       tex.repeat.set(6,6);
       // update ground material(s)
       if(floorVis && floorVis.material){
         floorVis.material.map = tex;
         floorVis.material.needsUpdate = true;
       }
       if(smallVis && smallVis.material){
         smallVis.material.map = tex;
         smallVis.material.needsUpdate = true;
       }
     });
   } catch(e){ console.warn('swapGround failed', e); }
 }
 function switchToMap2(){
   if(usingMap2) return;
   usingMap2 = true;
   _swapSkyTexture('/Actnko.png');
   _swapGroundTexture('/360_F_519151794_inO2QZz7WidXAGw2fafFJ9Hw0Zhnjj2X.jpg');
   if(map2Btn) map2Btn.style.display = 'none';
   if(map1Btn) map1Btn.style.display = '';
 }
 function switchToMap1(){
   if(!usingMap2) return;
   usingMap2 = false;
   _swapSkyTexture(originalSkyPath);
   _swapGroundTexture(originalGroundPath);
   if(map2Btn) map2Btn.style.display = '';
   if(map1Btn) map1Btn.style.display = 'none';
 }
 if(map2Btn) map2Btn.addEventListener('pointerdown', (e)=>{ e.preventDefault(); switchToMap2(); });
 if(map1Btn) map1Btn.addEventListener('pointerdown', (e)=>{ e.preventDefault(); switchToMap1(); });
...
document.addEventListener('click', (ev)=>{
  ...
  // Disallow placing into slot 2 entirely
  if(slotNum === 2){
    alert('Slot 2 is reserved and cannot hold items.');
    // move the selected item into inventory as a safe fallback, persist and update UI
    if(shopSelectedItem){
